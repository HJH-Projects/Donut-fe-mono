...
import { Heart, ArrowLeft, Share2, Trash2, Link2, Copy, X } from "lucide-react";
...
export function SharedLook() {
  ...
  // 현재 로그인한 사용자 (Mock - 실제로는 인증 시스템에서 가져옴)
  const currentUser = {
    id: "current-user-id",
    name: "나",
  };

  // Mock 데이터 로드 (실제로는 서버에서 lookId로 데이터 가져옴)
  useEffect(() => {
    // 실제 구현에서는 fetch(`/api/shared-looks/${lookId}`)
          ...
          >
            ...
            {!showLinkCreator ? (
              ...
              <div className="space-y-3">
                ...
                <button
                  onClick={() =>
                    handleCopyLink(sharedLinks[0].url, sharedLinks[0].id)
                  }
