...
export const getShareCommentsApi = (ky: KyInstance, path: string, options?: ApiRequestOptions) => {
  return ky.get(`shares/${path}/comments`, options).then((res) => res.json<S.CommentResponseDto[]>());
};
...
export const deleteShareCommentApi = (
  ...
  return ky.delete(`shares/${path}/comments/${id}`, options).then((res) => res.json<S.CommentDeleteResponseDto>());
};
